...
function ChargeCatalogPanel({ embedded = false }) {
  ...
  const fetchChargesList = async (refresh = false, searchOverride) => {
    if (refresh) {
    ...
    const searchTerm = searchOverride !== undefined ? searchOverride : search.trim();
  ...
  return (
    <div className="space-y-6">
            ...
            <tbody>
              {isLoading ? (
                ...
                charges.map((charge) => (
                  <tr
                    key={charge.id}
                    onClick={() => openChargeDetail(charge)}
      ...
      <ChargeDetailPanel
        ...
        chargePreviewName={selectedCharge?.charge_name || ""}
        onClose={closeChargeDetail}
        onAssignmentChange={() => fetchChargesList(true)}
      />
    </div>
  );
}

export default ChargeCatalogPanel;
